...
if($_GET[accion]=='asignar'){
	...
	$sql = mysqli_query($con, "SELECT COUNT(*) AS NUMBER_OF_ROWS FROM departamento_puesto WHERE id_puesto='$_POST[puesto]' AND id_departamento='$_POST[departamento]' AND id_usuario='$_POST[usuario]'");
	$row = mysqli_fetch_assoc($sql);
	$cols_respuesta = $row['NUMBER_OF_ROWS'];
...
if($_GET[accion]=='get_tickets_abiertas_usuario'){
	...
	$sql = mysqli_query($con,  "SELECT ut.id_usuario_ticket, t.id_ticket, t.nombre_ticket, ut.estado, t.descripcion, ut.nivel_prioridad, ut.creacion, ut.programada, ut.fecha_programada, ut.info_adicional, p.puesto, d.departamento, us.nombre_completo  FROM usuario us, usuario_ticket ut, ticket t, departamento_puesto dp, departamento d, puesto p WHERE t.id_ticket=ut.id_ticket AND ut.id_usuario=us.id_usuario AND us.id_usuario=dp.id_usuario AND dp.id_departamento=d.id_departamento AND dp.id_puesto=p.id_puesto AND (ut.estado=0 OR ut.id_calificacion IS NULL) AND us.id_usuario='$_POST[id_usuario]' GROUP BY ut.id_usuario_ticket");
...
if($_GET[accion]=='get_tickets_cerradas_soporte'){
	...
	$sql = mysqli_query($con,  "SELECT ut.id_usuario_ticket, t.id_ticket, t.nombre_ticket, ut.estado, t.descripcion, ut.nivel_prioridad, ut.creacion, ut.programada, ut.fecha_programada, ut.info_adicional, p.puesto, d.departamento, us.nombre_completo  FROM usuario us, usuario_ticket ut, ticket t, departamento_puesto dp, departamento d, puesto p WHERE t.id_ticket=ut.id_ticket AND ut.id_usuario=us.id_usuario AND us.id_usuario=dp.id_usuario AND dp.id_departamento=d.id_departamento AND dp.id_puesto=p.id_puesto AND ut.estado=1 AND ut.id_usuario='$_POST[id_usuario]' GROUP BY ut.id_usuario_ticket");
